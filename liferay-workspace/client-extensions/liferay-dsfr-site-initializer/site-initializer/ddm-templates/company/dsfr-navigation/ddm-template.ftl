...
<#else>
	...
	<div id="${navbarId}">
    <nav id="nav-${navbarId}" class="navbar navbar-expand-lg fr-nav" role="navigation" aria-label="Menu principal">
		<ul aria-label="<@liferay.language key="site-pages" />" class="nav navbar-nav navbar-site fr-nav__list" role="menubar">
			...
			<#list navItems as navItem>
				...
				<#if navItem.isBrowsable() || showChildrenNavItems>
					<#assign
						...
						nav_item_href_link = ""
						nav_item_link_css_class = "nav-link text-truncate fr-nav__link"
					/>
					...
					<#if showChildrenNavItems>
						...
						<#assign
							...
							nav_item_link_css_class = "${nav_item_link_css_class} fr-nav__btn dropdown-toggle"
						/>
					</#if>
